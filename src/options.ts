...
async function init() {
    ...
    for (let i = 0; i < optionsElements.length; i++) {
        ...
        switch (optionsElements[i].getAttribute("data-type")) {
            ...
            case "react-LabelOptionsComponent":
                new LabelOptions(optionsElements[i]);
                break;
        }
    }
...
function downloadConfig(element: Element) {
    ...
    file.setAttribute("download", `NoMoreSlop${optionType === "local" ? "OtherData" : "Config"}_${dateTimeString}.json`);
